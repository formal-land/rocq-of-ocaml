...
We present the attributes which we can use with `rocq-of-ocaml`. See the [attributes documentation](https://caml.inria.fr/pub/docs/manual-ocaml/attributes.html) of OCaml for general information about the attributes mechanism. Note that the OCaml attributes do not change the behavior of a program. There are there to help developer tools.

We prefix all the attributes of `rocq-of-ocaml` by `rocq_`. According to the OCaml syntax, depending on the context, you may use a single `@` or a double `@@`.

## rocq_axiom_with_reason
When we cannot import the definition of a value, we can use the `[@rocq_axiom_with_reason]` attribute to transform it to a Rocq axiom. For example:
```ocaml
let[@rocq_axiom_with_reason "mutable state"] function_hard_to_translate_to_coq =
  let n = ref 0 in
  fun () ->
    n := !n + 1;
    !n
...
```
Note that we must give a reason for the use of `[@rocq_axiom_with_reason]` in a string parameter. We define the `axiom` value in the `rocq-of-ocaml`'s Rocq library. The definition of `axiom` is:
```rocq
Axiom axiom : forall {A : Set}, A.
```

## rocq_cast
With the attribute `[@rocq_cast]` we can force the type in Rocq of an arbitrary OCaml expression using the following axiom:
```rocq
...
Definition f {a : Set} (kind : t) (x : a) : int :=
  ...
  Z.add (cast int x) 1.
```
Thanks to the `cast` axiom, we can get the information in Rocq that `x` is of type `int`. Without this axiom the example would not work. Indeed, we do not track the type equations generated by the [GADTs](https://caml.inria.fr/pub/docs/manual-ocaml/gadts.html) so `x` would be considered of type `a`. Without the cast, the Rocq code:
```rocq
Definition f {a : Set} (kind : t) (x : a) : int :=
  let 'Int := kind in
  Z.add x 1.
...
## rocq_force_gadt
The `[@rocq_force_gadt]` attribute forces a type definition to be considered as a GADT during the translation to Rocq. In particular, it forces the translation to erase the type parameters. For example:
```ocaml
...
One possible reason to force a type to be a GADT is to make sure that all the inductive types in a mutually recursive type definition have the same (zero) arity, as it is expected by Rocq.

## rocq_grab_existentials
...
To achieve this we use the `[@rocq_grab_existentials]` attribute. Here is an example:
...
let w2_of_w1 (w : wrap2) : wrap1  =
  match [@rocq_grab_existentials]w with
  | Cw2 f ->
    Cw1 (fun y -> f y)
```

Notice that the type of `inj` is `'a -> 'a` for some existential variable `'a`.
Since `rocq-of-ocaml` always generates fully anotated code, we need to explicitely
...
On the Rocq side we use an `existT` to grab these existential variables.  The key
...
## rocq_implicit
The `[@rocq_implicit "(A := ...)"]` attribute adds an arbitrary annotation on an OCaml identifier or constructor. We typically use this attribute to help Rocq to infer implicit types where there is an ambiguity:
```ocaml
let forget x = ()

let u = forget []
```
generates the following Rocq code:
```rocq
...
```ocaml
let u = forget ([] [@rocq_implicit "(A := unit)"])
```
so that the generated Rocq code compiles:
```rocq
Definition u : unit := forget (nil (A := unit)).
```

## rocq_match_gadt
The `[@rocq_match_gadt]` attribute adds type annotations for the pattern variables in a `match`. We force the type annotations to be valid using axioms (dynamic casts). For example:
```ocaml
type 'a int_or_bool =
  | Int : int int_or_bool
...
Note that without the `[@rocq_match_gadt]` attribute this would generate:
```rocq
Definition to_int {a : Set} (kind : int_or_bool) (x : a) : Z :=
  match (kind, x) with
  | (Int, _ as x) => x
...
```
which is ill-typed in Rocq.

## rocq_match_gadt_with_result
The attribute `[@rocq_match_gadt_with_result]` is similar to `[@rocq_match_gadt]` and also adds a cast for the result of each `match` branch. Here is an example where it is useful:
```ocaml
let incr_if_int (type a) (kind : a int_or_bool) (x : a) : a =
  match[@rocq_match_gadt_with_result] (kind, x) with
  | (Int, (x : int)) -> x + 1
  | (Bool, (x : bool)) -> x 
```
generates in Rocq:
...
## rocq_match_with_default
The `[@rocq_match_with_default]` adds a default branch for `match` which are syntactically incomplete. For example, when we annotate the following code:
```ocaml
let incr_int (kind_and_value : int int_or_bool * int) : int =
  match[@rocq_match_with_default] kind_and_value with
  | (Int, x) -> x + 1
```
we generate the following valid Rocq code:
```rocq
...
We can combine this attribute with `[@rocq_match_gadt]` or `[@rocq_match_gadt_with_result]` if needed.

## rocq_mutual_as_notation
The attribute `[@rocq_mutual_as_notation]` makes the definition of a mutually recursive function a notation. For example, we transform the following OCaml code:
```ocaml
...
```
which is rejected by the type-checker of Rocq:
```
Error:
...
For recursive notations, you can combine this attribute with `@rocq_struct` to tell `rocq-of-ocaml` to generate a recursive notation. For example, we transform:
```ocaml
type 'a tree =
  | Leaf of 'a
  | Node of 'a tree list
...
```
using the keyword `fix` for the defintion of `sums`. In this example too, the type-checker of Rocq would reject the definition without a notation.

> In the proofs, whenever the definition of `sums` appears unfolded, you can run the tactic `fold sums` to hide it.

## rocq_phantom
When it can, `rocq-of-ocaml` detects phantom types and remove their type annotations. For example, we translate the following OCaml code:
```ocaml
type 'a num = int
...
```
The reason is that phantom types may generate ambiguities during type inference in Rocq.

The `[@rocq_phantom]` attribute forces `rocq-of-ocaml` to consider a type as phantom. This can be useful for abstract types in `.mli` files, since their definition is not reachable. For example:
```ocaml
type 'a num
```
translates to:
...
## rocq_plain_module
We may prefer to translate a module to a plain Rocq module rather than a record. The `[@rocq_plain_module]` attribute requires a module to be translated as a plain Rocq module. For example:
```ocaml
module type T = sig
  type t
...
Definition M :=
  ...
  existT (A := unit) (fun _ => _) tt
    ...
    |}.
```
With the `[@rocq_plain_module]` attribute we translate:
```ocaml
module type T = sig
  type t
...
module[@rocq_plain_module] M = struct
  type t = int
...
In order to distinguish between two signatures with the same value names, we can add the `[@rocq_precise_signature]` attribute. For example, we can translate:
```ocaml
...
```ocaml
let[@rocq_struct "accumulator"] rec length l accumulator =
  match l with
  | [] -> accumulator
...
To see its usefulness translating impossible branches without extra axioms check `rocq_tagged_match`

## rocq_tagged_match
With the `rocq_tag_gadt` attribute we can translate OCaml code closer to its actual semantics. This allows us to translate pattern matches with impossible branches without the use of axioms. For example:
...
## rocq_type_annotation
Sometimes we need to add a type annotation on an expression, either as a documentation or to help the Rocq code to compile. We translate this OCaml example:
```ocaml
let n1 =
  let m = 12 in
