...
> Dropping type parameters in Rocq may incur some issues to infer implicit type variables. We recommend to add type annotations with the OCaml attribute `[@rocq_implicit "(A := _)"]` when Rocq does not achieve to infer type variables.

To compile the pattern-matching in GADT mode we require an attribute `[@rocq_match_gadt]` in OCaml:
```ocaml
let to_string (type a) (kind : a int_or_string) (x : a) : string =
  match[@rocq_match_gadt] kind, x with
  | Int, (x : int) -> string_of_int x
...
With this in mind we provide a tagging mechanism to achieve this through the flag `[@@rocq_tag_gadt]`
...
let rec get_int (e : int term) : int =
  ...
  | _ -> .
```
Please notice that we have to indicate that we are pattern matching against a tagged GADT with the flag `rocq_tagged_match`.
...
For more details on the mechanisms behind this translation please check [this blog post](https://pedroabreu0.github.io/blog/2020/08/05/OCaml-GADTs-In-Rocq)
