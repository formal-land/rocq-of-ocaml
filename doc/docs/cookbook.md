...
Here we list typical situations where we need to change the OCaml source code so that the translated code compiles in Rocq.

## Abstractions in `.mli` files
When generating the Rocq code, we do not use the notion of `.mli` because there are no such interface files in Rocq. So the typical setup is to generate a `.v` file for each `.ml` file, and only translate the `.mli` files of the external dependencies to axiom files.
...
The way to specify the decreasing parameter is to use the attribute [rocq_struct](attributes#rocq_struct). For example we annotate the operator `-->` as follows:
```ocaml
let[@rocq_struct "i"] rec ( --> ) i j =
  (* [i; i+1; ...; j] *)
  if Compare.Int.(i > j) then [] else i :: (succ i --> j)
```
Here `i` is decreasing when we consider the natural order on `-i`. This generates the following Rocq code:
```rocq
...
```
The annotation `{struct i}` specifies in Rocq that the decreasing parameter is `i`.

## Ignored functions
Sometimes definitions are too complex to translate to Rocq, but we still want to go on with the rest of the files. A solution is to add the [rocq_axiom_with_reason](attributes#rocq_axiom_with_reason) to ignore a definition and replace it with an axiom of the same type.

For example, the following definition would not work in Rocq as is it is, due to the use of GADTs:
```ocaml
  let fold_all f set acc =
    List.fold_left
      ...
      all
```
We then add the attribute `@rocq_axiom_with_reason`:
```ocaml
let fold_all f set acc =
    List.fold_left
      (fun acc (_, Ex_Kind kind) -> fold kind (f.f kind) set acc)
      acc
      all
    [@@rocq_axiom_with_reason "gadt"]
```
This generates the following Rocq code:
```rocq
...
Sometimes mutual definitions for a recursive function are used more as notations rather than to express a true mutual recursion. See the attribute [rocq_mutual_as_notation](attributes#rocq_mutual_as_notation) for more details about how to handle this kind of definition. Here is an example where this attribute is needed:
```ocaml
...
```
We replace it by the following OCaml code, which translates into Rocq without errors:
```ocaml
...
| Gt : int -> depth.
```
Then, using the configuration parameters [variant_constructors](configuration#variant_constructors) and [variant_types](configuration#variant_types), we instruct `rocq-of-ocaml` to recognize that there is a type `depth` whenever it finds a constructor `` `Eq``, ..., or `` `Gt`` in the OCaml code.
...
```
to show the definition of `matching_function` first. This example would not work in Rocq because mutually recursive definitions have to be with at least one algebraic type definition. Even for cases where the translation works, having too many mutually recursive type definitions may complexify the proofs.
...
In Rocq, it is not possible to have two definitions of the same name at top-level. For example, if we translate the following OCaml code:
```ocaml
